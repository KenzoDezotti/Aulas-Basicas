...
class Jogo(object):
    ...
    def Salvar(self):
        ...
        Salva o jogo atual
        """
        db = shelve.open('jogo.db')
        db["jogador"] = self.player 
        db["inimigo"] = self.inimigos
    ...
    def carregaJogo(self):
        ...
        try:
            db = shelve.open('jogo.db')
            self.player = db["jogador"] 
            self.inimigos = db['inimigo']
            db.close()
        except:
            print("não encontrei arquivo carregado, bora começar do zero")
            self.novoJogo()
